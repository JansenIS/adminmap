...
function vk_bot_generate_character_image(string $userPrompt): array {
  ...
  if ($b64 !== '') {
    ...
    if (is_string($raw) && $raw !== '') return ['ok' => true, 'raw' => $raw, 'http_code' => $routerCode, 'router_response' => substr((string)$resp, 0, 4000)];
  }
  if ($imageUrl !== '') {
    ...
    if (is_string($raw) && $raw !== '' && $downloadCode < 400 && $downloadErr === '') {
      return ['ok' => true, 'raw' => $raw, 'http_code' => $routerCode, 'router_response' => substr((string)$resp, 0, 4000)];
    }
    return ['ok' => false, 'error' => 'image_download_failed', 'http_code' => $routerCode, 'router_response' => substr((string)$resp, 0, 4000)];
  }

  vk_bot_log_error('routerai_image_not_found body=' . substr($resp, 0, 1200));
  return ['ok' => false, 'error' => 'image_not_found', 'http_code' => $routerCode, 'router_response' => substr((string)$resp, 0, 4000)];
}
