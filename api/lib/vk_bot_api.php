...
function vk_bot_is_free_province(array $prov, string $territoryType): bool {
  ...
  // Провинция не считается свободной, если уже закреплена за правителем/сеньором.
  $hasDirectControl = trim((string)($prov['owner'] ?? '')) !== ''
    || trim((string)($prov['suzerain'] ?? '')) !== ''
    || trim((string)($prov['senior'] ?? '')) !== '';
  if ($hasDirectControl) return false;
...
function vk_bot_free_provinces_for_territory(array $state, string $type, string $id): array {
  ...
  foreach (($state['provinces'] ?? []) as $pid => $prov) {
    ...
    if (!vk_bot_is_free_province($prov, $type)) continue;
