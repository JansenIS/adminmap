...
if ($method === 'POST') {
  ...
  $order = [
    ...
    'rp_post'=>mb_substr(trim((string)($p['rp_post'] ?? '')),0,20000),'public_images'=>array_values(array_filter((array)($p['public_images'] ?? []), static fn($v)=>is_string($v)&&$v!=='')),
    'private_attachments'=>array_values(array_filter((array)($p['private_attachments'] ?? []), static fn($v)=>is_string($v)&&$v!=='')),
