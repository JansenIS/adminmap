...
function renderPreview(){
  ...
  lines.push('Ход/год: ' + ((current&&current.turn_year)?current.turn_year:'авто (текущий ход)'));
  lines.push('\nРП-пост:\n' + (q('#rpPost').value||'—'));
  lines.push('\nПункты:');
...
function applyOrderToForm(o){
  ...
  q('#turnYearInfo').textContent = o.turn_year ? ('Год: ' + o.turn_year) : 'Год будет выставлен автоматически';
...
function newOrder(){
  ...
  privateAttachments=[];
  q('#title').value='';
  q('#turnYearInfo').textContent='Год будет выставлен автоматически';
  q('#rpPost').value='';
  drawItems([]);
  drawAttachments();
  fillAttachmentForm(null);
