...
function openOrder(o){
  ...
  (o.action_items||[]).forEach(it=>{
    ...
    row.innerHTML=`<div><b>${it.category}</b> — ${it.summary}</div><div class='muted'>${it.details||''}</div><div class='row'><input class='mod' type='number' value='0'><button class='roll'>Бросить d20</button><div class='muted'>${roll?('total '+roll.total+' '+roll.outcome_tier):'нет броска'}</div></div>`;
