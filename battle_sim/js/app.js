(function(){
  ...
  const tokenRequired = String(qs.get('token_required') || '').trim() === '1';
  ...
  function unitBattleUid(u){
    return String(u && (u._battleUid || ((u._battleArmyUid || '') + '#' + String(Number(u._battleUnitIdx)||0))) || '').trim();
  }
...
if(battle.started && battle.phase==="ranged" && picked){
  ...
  if(tokenMode.enabled){
    const auid = unitBattleUid(shooter);
    const tuid = unitBattleUid(target);
...
if(battle.started && battle.phase==="ranged" && sel && sel.stats.ranged && clicked && clicked.side!==sel.side && clicked.state!=="destroyed"){
  ...
  if(!canShoot){
    ...
    if(!E.canTargetByRangedBoundary(sel, clicked)){
      ...
      if(tokenMode.enabled){
        const auid = unitBattleUid(sel);
        const tuid = unitBattleUid(clicked);
        sendBattleActions([{ type:'ranged_attack', uid: auid, target_uid: tuid }])
          .then(async ()=>{ await loadTokenBattleScenario({ hydrate: true }); refreshAll(); })
          .catch((e)=>{ E.log('Выстрел отклонён сервером: ' + (e && e.message ? e.message : e), 'mut'); refreshAll(); });
      }else{
...
refreshAll();
      ...
      if(useRealtime){
        ...
        for(const row of realtimeUnits){
          ...
          if(preserveOwnSetupPose && String(u.side || '') === ownSide){
            const savedPose = preserveOwnPoseByUid.get(unitBattleUid(u));
            if(savedPose){
  ...
  const lockSimulatorForTokenRequirement = ()=>{
    const ids = ['btnStart','btnNext','btnReset','addUnit','btnHeal','btnKill','btnRemove','btnChangeFormationMove'];
    for(const id of ids){
  ...
  const renderTokenHintCard = (messageHtml)=>{
  ...
  async function bootstrap(){
    ...
    if(!tokenMode.enabled){
      if(tokenRequired){
        renderTokenHintCard('Режим токен-сессии обязателен (<code>token_required=1</code>), но токен не распознан. Откройте <code>/battle_sim/token_sim.html?token=...</code>.');
        E.log('Токен-сессия обязательна, но токен не найден.', 'bad');
        lockSimulatorForTokenRequirement();
      } else {
        renderNoTokenHint();
      }
    }
    if(tokenMode.enabled){
      ...
      try {
        ...
        if(tokenRequired){
          E.log('Обычный симулятор отключён: token_required=1.', 'bad');
          lockSimulatorForTokenRequirement();
        }
      }
    }
