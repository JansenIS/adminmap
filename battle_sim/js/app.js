...
refreshAll();
  ...
  function renderNoTokenHint(){
    ...
    card.className = 'card';
    card.id = 'tokenSessionCard';
    card.innerHTML = '<div class="cardTitle">Токен-сессия боя</div><div class="cardBody"><div class="small">Токен не найден. Откройте battle_sim/token.html?token=... или добавьте в URL index.html параметр <b>?battle_token=...</b>.</div></div>';
    if(panel.firstChild) panel.insertBefore(card, panel.firstChild);
    else panel.appendChild(card);
  }
