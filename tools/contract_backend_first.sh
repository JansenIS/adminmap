...
curl -sS -o /tmp/adminmap_contract_emblems_ifmatch_required.json -w '%{http_code}' -X POST 'http://127.0.0.1:8001/api/assets/emblems/' -H 'Content-Type: application/json' --data '{"migrate":true}' | python3 -c 'import sys;assert sys.stdin.read().strip()=="428"'
python3 - <<'PYC'
import json
from pathlib import Path
d=json.loads(Path('/tmp/adminmap_contract_emblems_ifmatch_required.json').read_text())
...
# contract_invalid_ifmatch_type
curl -sS -o /tmp/adminmap_contract_invalid_ifmatch_type.json -w '%{http_code}' -X PATCH 'http://127.0.0.1:8001/api/provinces/patch/' -H 'Content-Type: application/json' --data '{"pid":1,"if_match":123,"changes":{"terrain":"x"}}' | python3 -c 'import sys;assert sys.stdin.read().strip()=="400"'
python3 - <<'PYC'
import json
from pathlib import Path
d=json.loads(Path('/tmp/adminmap_contract_invalid_ifmatch_type.json').read_text())
assert d.get('error')=='invalid_payload_type'
assert d.get('field')=='if_match'
PYC

# contract_nested_schema
curl -sS -o /tmp/adminmap_contract_nested_schema.json -w '%{http_code}' -X POST 'http://127.0.0.1:8001/api/changes/apply/' -H 'Content-Type: application/json' -H "If-Match: ${V}" --data '{"changes":[{"kind":"province","pid":1,"changes":{"fill_rgba":[1,2,"bad",4]}}]}' | python3 -c 'import sys;assert sys.stdin.read().strip()=="400"'
python3 - <<'PYC'
import json
from pathlib import Path
d=json.loads(Path('/tmp/adminmap_contract_nested_schema.json').read_text())
