...
- Базовая commodity-модель синхронизирована с `resources.js`: в PHP загружается полный список товаров из отдельного файла `php/commodities.php`.
- Файлы основного проекта не изменяются симулятором, чтение только read-only.
- Если проект развернут в нестандартной структуре директорий, можно задать переменную окружения `ISOTOPE_PROJECT_ROOT` до корня, где лежат `data/`, `hexmap/`, `isotope/`.
...
- В `summary` и `province` добавлены поля казны провинций (`treasury*`/`treasury`) и годовой дельты.
- `api.php?action=trade-history&year=Y&limit=200` — история заявок/сделок внутренней и внешней торговли за год.
...
- Добавлен этап межпровинциальной логистики как полного min-cost flow по графу дистанций для балансировки дефицитов/профицитов по каждому товару за шаг.
- Добавлена провинциальная денежная экономика (казна): ежегодные поступления от налогов поселений, налогов с торговли и транзитных сборов, с умеренными расходами/антисноуболл-механикой для сдерживания экспоненциального роста.
- Внутренняя торговля переведена на ордербук: частичное исполнение, приоритет цены/времени, перенос неисполненного остатка заявок на следующий год (до истечения TTL).

## Не успел перенести (пока)

- Полная рыночная механика ордербука с типами заявок (market/limit/IOC/FOK), отдельной книгой внешнего рынка и сквозным денежным клирингом (сейчас реализован базовый limit matching для internal + агрегированный external).
