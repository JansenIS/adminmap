...
- Добавлен упрощённый промышленный граф (несколько производственных рецептов input/output) и отдельный логистический этап перераспределения по кратчайшим путям между провинциями.

## Не успел перенести (пока)

- Полный промышленный граф из оригинального `resources.js`/`BUILDINGS` (с покрытием всех рецептов и их точных параметров): сейчас перенесена только часть рецептов и упрощённые коэффициенты.
- Точный расчёт логистики по путям (distance/стоимость на путь с оптимизацией по графу), сейчас используется соседская торговля + внешний арбитраж как аппроксимация.
