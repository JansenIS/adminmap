...
function layout() {
  ...
  componentMeta.forEach(({ idx }) => {
    ...
    for (let pass = 0; pass < 2; pass++) {
      // Важно: приоритет — держать супругов рядом. Любые "блоки по родителям" на взрослом ряду
      // легко разрывают браки. Поэтому финальный этап — уплотнение spouse-блоков.
...
function render({ preserveViewportAnchor = true } = {}) {
  ...
  // Канонизация spouse-связей по паре: иначе при дубликатах/несогласованных union_id
  // получаются параллельные линии и раздувается число треков.
  const familyUnionIdsByPair = new Map();
  const layoutFamiliesForCanon = Array.isArray(state.layoutFamilies) ? state.layoutFamilies : [];
  layoutFamiliesForCanon.forEach((f) => {
    if (!f?.parentA || !f?.parentB) return;
    const pairKey = relKey(f.parentA, f.parentB);
    const unionId = String(f.unionId || '').trim();
    if (!unionId) return;
    if (!familyUnionIdsByPair.has(pairKey)) familyUnionIdsByPair.set(pairKey, new Set());
    familyUnionIdsByPair.get(pairKey).add(unionId);
  });

  const spouseByPair = new Map();
  state.relationships
    .filter(r => r.type === 'spouses')
    .forEach((r) => {
      const pairKey = relKey(r.source_id, r.target_id);
      if (!spouseByPair.has(pairKey)) spouseByPair.set(pairKey, []);
      spouseByPair.get(pairKey).push(r);
    });

  const canonicalSpouseRelByPair = new Map();
  spouseByPair.forEach((rels, pairKey) => {
    const preferred = familyUnionIdsByPair.get(pairKey);
    let chosen = null;

    if (preferred && preferred.size) {
      chosen = rels.find(rr => preferred.has(String(rr.union_id || '').trim()));
    }
    if (!chosen) {
      chosen = rels
        .slice()
        .sort((a, b) => String(a.union_id || '').localeCompare(String(b.union_id || '')))[0] || null;
    }
    if (chosen) canonicalSpouseRelByPair.set(pairKey, chosen);
  });

  const spouseIntervals = [];

  // sibling-интервалы: используем как есть (они не имеют union_id).
  state.relationships.forEach((r) => {
    if (r.type !== 'siblings') return;
    const a = state.positions.get(r.source_id);
    const b = state.positions.get(r.target_id);
    if (!a || !b || Math.abs(a.y - b.y) >= 1) return;
    siblingIntervals.push({
  ...
  // spouse-интервалы: только канонические пары.
  canonicalSpouseRelByPair.forEach((r, pairKey) => {
    const a = state.positions.get(r.source_id);
    const b = state.positions.get(r.target_id);
    if (!a || !b || Math.abs(a.y - b.y) >= 1) return;
    spouseIntervals.push({
      id: pairKey,
      l: a.x,
      r: b.x,
      generation: generationById.get(r.source_id) ?? 0,
