...
- Панель игрока по токену: `http://<SERVER_IP>:8080/player.html?token=...`
...
## Игровая панель по односуточному токену

- Админ создаёт ссылку в `admin.html` в блоке **«Ссылка игрока на 24 часа»** (тип + ID сущности).
- Backend выдаёт постоянный токен-ссылку на 24 часа (`/api/player/tokens/create/`).
- Игрок открывает `player.html?token=...` и получает доступ только к своей сущности/провинциям:
  - смена герба сущности и доменных провинций;
  - редактирование описания и изображения сущности/провинций;
  - созыв арьербана (с лимитом по уровню сущности);
  - передвижение и роспуск созванных армий без потерь;
  - просмотр казны и населения (суммарно и по провинциям).
  - просмотр карты в публичных режимах слоёв (королевства/дома/города/территории) с отображением всех чужих провинций.

Новые API:
- `POST /api/player/tokens/create/`
- `GET /api/player/session/?token=...`
- `POST /api/player/session/save/`
- `POST /api/player/army/action/`
