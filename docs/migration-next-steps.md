...
- [~] Строгая schema validation для всех PATCH/batch payloads. (добавлены strict checks для province/realm PATCH, changes/apply, migration/* и assets/jobs write payloads; требуется полная схема для всех write endpoints)
